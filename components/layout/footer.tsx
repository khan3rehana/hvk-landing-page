      ...
      <ul className="mt-4 space-y-2.5">
        {links.map((link) => (
          <li key={`${title}-${link.label}`}>
            <Link
              href={link.href}
              className="text-sm text-white/70 transition-colors hover:text-bright-blue"
            >
              {link.label}
            </Link>
          </li>
  ...
  return (
    <footer className="bg-navy text-white" id="contact">
            ...
            {SOCIAL_LINKS.map((social) => (
              <li key={social.label}>
                <a
                  ...
                  className="flex h-9 w-9 items-center justify-center rounded-full border border-white/15 text-white/80 transition-colors hover:border-bright-blue hover:bg-bright-blue hover:text-white"
                >
      ...
      <div className="border-t border-white/10">
        <div className="container flex flex-col items-center justify-between gap-3 py-5 text-xs text-white/60 sm:flex-row">
